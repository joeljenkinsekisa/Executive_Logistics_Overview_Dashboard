...
DROP VIEW IF EXISTS executive_logistics_overview_view;

CREATE VIEW executive_logistics_overview_view AS
SELECT
    l.load_id,
    l.load_date,
    DATE_FORMAT(l.load_date, '%Y-%m') AS load_month,
    ...
    COALESCE(de.total_delivery_events, 0) AS total_delivery_events,
    COALESCE(de.on_time_events, 0) AS on_time_events,
    COALESCE(de.late_events, 0) AS late_events,
    COALESCE(de.avg_detention_minutes, 0) AS avg_detention_minutes
...
LEFT JOIN (
    SELECT
        ...
        SUM(on_time_flag) AS on_time_events,
        COUNT(*) - SUM(on_time_flag) AS late_events,
        AVG(detention_minutes) AS avg_detention_minutes
    FROM delivery_events
    GROUP BY load_id
) de ON l.load_id = de.load_id;